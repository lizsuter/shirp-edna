...
asv_taxonomya <- read.delim("/Volumes/easystore/eDNA/shirp-edna/results-revamp-2024-CO1a_Chordata_reBLAST/results-revamp-2024-CO1a_Updated_asvTaxonomyTable_fixed.txt", header=TRUE, stringsAsFactors=FALSE)
asv_taxonomyb <- read.delim("/Volumes/easystore/eDNA/shirp-edna/results-revamp-2024-CO1b_Chordata_reBLAST/results-revamp-2024-CO1b_Updated_asvTaxonomyTable_fixed.txt", header=TRUE, stringsAsFactors=FALSE)
asv_taxonomyc <- read.delim("/Volumes/easystore/eDNA/shirp-edna/results-revamp-2024-CO1c_Chordata_reBLAST/results-revamp-2024-CO1c_Updated_asvTaxonomyTable_fixed.txt", header=TRUE, stringsAsFactors=FALSE)
...
```{r}
# of reads including unknowns
sum(rowSums(asv_count_unknowns_2024_exp_mifish))
...
```{r}
# of reads including unknowns
sum(rowSums(asv_count_unknowns_2024_exp_elas))
...
asv_count_unknowns_2024_exp_co1 <- full_join(asv_count_unknowns_2024_exp_co1_a, asv_count_unknowns_2024_exp_co1_b) %>% full_join(asv_count_unknowns_2024_exp_co1_c) 
...
asv_count_unknowns_2024_exp_co1 <- asv_count_unknowns_2024_exp_co1 %>%
  select(-c("MP_C_2_S1_L001","MP_C_3_S1_L001","MP_C_4_S1_L001","MP_C_5_S1_L001"))
...
```{r}
# of reads including unknowns
sum(rowSums(asv_count_unknowns_2024_exp_co1, na.rm = TRUE))
...
```{r}
# of reads including unknowns
sum(rowSums(asv_count_unknowns_2024_trawl_mifish, na.rm = TRUE))
...
```{r}
# of reads including unknowns
sum(rowSums(asv_count_unknowns_2024_trawl_elas, na.rm = TRUE))
...
```{r}
## With unknowns--
asv_count_unknowns_2024_trawl_co1_a <- read.delim("/Volumes/easystore/eDNA/shirp-edna/results-revamp-2024-CO1a/dada2/ASVs_counts.tsv", header=TRUE, stringsAsFactors=FALSE)
asv_count_unknowns_2024_trawl_co1_b <- read.delim("/Volumes/easystore/eDNA/shirp-edna/results-revamp-2024-CO1b/dada2/ASVs_counts.tsv", header=TRUE, stringsAsFactors=FALSE)
asv_count_unknowns_2024_trawl_co1_c <- read.delim("/Volumes/easystore/eDNA/shirp-edna/results-revamp-2024-CO1c/dada2/ASVs_counts.tsv", header=TRUE, stringsAsFactors=FALSE)

# ASV numbers are unique to each of the subsets of data and overlap across datasets. Put a marker in ASV name to indicate whether it's from library a, b, or c
asv_count_unknowns_2024_trawl_co1_a <- asv_count_unknowns_2024_trawl_co1_a %>% mutate(x = paste0(x, "_a"))
asv_count_unknowns_2024_trawl_co1_b <- asv_count_unknowns_2024_trawl_co1_b %>% mutate(x = paste0(x, "_b"))
asv_count_unknowns_2024_trawl_co1_c <- asv_count_unknowns_2024_trawl_co1_c %>% mutate(x = paste0(x, "_c"))

asv_count_unknowns_2024_trawl_co1 <- full_join(asv_count_unknowns_2024_trawl_co1_a, asv_count_unknowns_2024_trawl_co1_b) %>%
  full_join(asv_count_unknowns_2024_trawl_co1_c) 

row.names(asv_count_unknowns_2024_trawl_co1) <- asv_count_unknowns_2024_trawl_co1$x
asv_count_unknowns_2024_trawl_co1 <- asv_count_unknowns_2024_trawl_co1 %>% select(-x)



# Remove control samples:
asv_count_unknowns_2024_trawl_co1 <- asv_count_unknowns_2024_trawl_co1 %>%
  select(-c("MP_STPositive_S1_L001","MP_T7Blank_S1_L001","MP_S_Blank_S1_L001","MP_S_Positive_S1_L001"))

## Without unknowns--
asv_count_nounknowns_2024_trawl_co1 <- otu_table(subset_samples(phylo_2024_trawl_co1, is.na(sample_data(phylo_2024_trawl_co1)$controls)))
...
```{r}
# of reads including unknowns
sum(rowSums(asv_count_unknowns_2024_trawl_co1, na.rm = TRUE))

# number of ASVs retained after removal of unknowns
nrow(asv_count_nounknowns_2024_trawl_co1)

# fraction of ASVs retained after removal of unknowns
nrow(asv_count_nounknowns_2024_trawl_co1)/nrow(asv_count_unknowns_2024_trawl_co1)

# number of reads retained after removal of unknowns
sum(rowSums(asv_count_nounknowns_2024_trawl_co1))

# fraction of reads retained after removal of unknowns
sum(rowSums(asv_count_nounknowns_2024_trawl_co1))/sum(rowSums(asv_count_unknowns_2024_trawl_co1, na.rm = TRUE))
...
CO1: 25.1% of ASVs were removed as unknowns (76.5% of total reads). 34270 ASVs retained (7,521,817 reads). 
...
#### Trawl-CO1
```{r}
ranks <- rank_names(phylo_2024_trawl_co1)
ranks

as.data.frame(unique(tax_table(phylo_2024_trawl_co1)[, ranks[1]]))
as.data.frame(unique(tax_table(phylo_2024_trawl_co1)[, ranks[2]]))
as.data.frame(unique(tax_table(phylo_2024_trawl_co1)[, ranks[3]]))
...
unique(tax_table(ps2024_trawl_co1)[, ranks[1]])
```
...
Mammals are target of these primers, but I don't want to retain non-target mammals like mouse, dog, human, etc. Check
```{r}
mammals_2024_trawl_co1 <- subset_taxa(ps2024_trawl_co1, Class == 'Mammalia')
unique(tax_table(mammals_2024_trawl_co1)[, ranks[7]])
...
nrow(tax_table(ps2024_trawl_co1)) # number of taxa left
as.data.frame(unique(tax_table(ps2024_trawl_co1)[, ranks[7]]))
...
nrow(tax_table(ps2024_exp_co1)) # number of taxa left
as.data.frame(unique(tax_table(ps2024_exp_co1)[, ranks[4]]))
as.data.frame(unique(tax_table(ps2024_exp_co1)[, ranks[7]]))
...
nrow(tax_table(ps2024_trawl_co1)) # number of taxa left
as.data.frame(unique(tax_table(ps2024_trawl_co1)[, ranks[4]]))
as.data.frame(unique(tax_table(ps2024_trawl_co1)[, ranks[7]]))
...
#### Summary Diveristy stats- USV vs Manual
After QC and curation, these are the number of ASVs left:
Expedition/MiFish: 1148
Expedition/Elas02: 1229
Expedition/CO1: 39,617
SeptemberManual/MiFish: 1414
SeptemberManual/Elas02: 1419
SeptemberManual/CO1: 34,216
...
Expedition/CO1: 509
SeptemberManual/MiFish: 31
SeptemberManual/Elas02: 15
SeptemberManual/CO1: 516

After QC and curation, these are the number of unique ASVs left:
...
SeptemberManual/MiFish: 80
SeptemberManual/Elas02: 28
SeptemberManual/CO1: 1035 (ditto)
...
sum(apply(otu_table(ps2024_trawl_co1), 2, as.numeric))
...
df.neg.ASVs[rownames(neg_controls_sorted),]
```
These are mostly protists/ algae/ fungi/ insects. Great. It was not a sterile control so this is fine. No fish/ marine animals
...
A lot of algae/ protists and arthropods- Pos control species are very low compared to this contamination (not really contamination bc these are fish tank samples- so prob real).

I decided not to remove any from CO1. Because of the much higher diversity, many of the species are <500 reads.

```{r}
data.frame(otu_table(ps2024_trawl_co1)) # compare before
```
34,216 taxa remain
...
```
## CONTINUE INTEGRATING CO1-TRAWL PHYLOSEQ OBJECT FROM HERE 11/26/25
