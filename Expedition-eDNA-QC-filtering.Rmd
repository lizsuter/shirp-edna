...
table5 <- otu_table(phylo_2024_exp_co1)
class(table5) <- "matrix" 
table5 <- t(table5) 
...
rarecurve(table5, step=50, cex=0.5)
...
```
None of the more abundant ASVs are in the negative controls, that's great. The max read abundance for an ASV in a negative control is 37931, which is high but not too relatively high for this library. What are these ASVs (don't appear in table preview above)--
...
```
These are mostly protists/ algae/ fungi. Great.
...
Exp/CO1-
NOTE for CO1, agglomerate by Species, not common name, because there is way too much diversity here to assign a common name, and there are many species which do not have one.
```{r}
