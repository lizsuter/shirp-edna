...
- 1. Community-level Parameters (average effect of covariates across all species)
  ...
  - If a credible interval does not exclude zero, there’s no strong community-level signal. For example: if Avg_Temperature_C mean = 0.84, but CI: -1.79 to 3.85 --> inconclusive.
  - Rhat values > 1.1, and low ESS (effective sample size) are a sign of poor convergence, so treat with caution
- 2. Species-specific Occupancy Effects. These show how each ASV’s occupancy probability changes per unit change in the covariate.
  - Example:
...
- 4. Variance Terms. These describe variation among species in their responses to covariates.
  ...
  - If convergence is poor (Rhat > 1.1, ESS < 100), don’t over-interpret.
...
- Slightly low/high values (e.g., 0.07–0.10 or 0.90–0.93) suggest potential mild misfit, but aren't typically cause for major concern.
...
- Species-level p-values:
  ...
  - Others: all between 0.15–0.21 → well within reasonable bounds.
...
- Species-level p-values:
  - ASV_1 & ASV_2: ~0.07–0.08 → slightly low but still acceptable.
  - ASV_7–14: 0.25–0.47 → well within acceptable bounds.
Interpretation:
...
Since this is a single model, the WAIC alone isn’t meaningful unless comparing it to WAIC from another model.

2. elpd (Expected Log Predictive Density) = -20.91
Measures the model’s predictive accuracy: how well it predicts new data.
...
Reflects the model’s complexity.
...
In particular, the assumption of independence between sites may obscure meaningful spatial patterns. Given the likely spatial autocorrelation in elasmobranch distributions and the fine-scale environmental gradients present in the estuarine system, transitioning to a spatial occupancy framework is warranted. Incorporating spatial random effects or spatially structured latent factors can improve the model’s ability to detect true ecological effects and provide more robust insight into the drivers of elasmobranch occurrence and detection dynamics.
...
Solution-- use `grid.index` which tells the model to apply the spatial effect at the site level, even if there are multiple samples per site. Preserves temporal replication while satisfying the model’s spatial requirements.
...
Summarize results
I've been iteratively adjusting parameters in model above to optimize model convergence (Rhat, ESS), and SDs for means/ variances. Variances are no longer extreme (e.g., ~1–2, not 100+).
```{r}
...
- Model Convergence Is Excellent All Rhat values are ~1.00–1.01 — even species-level parameters.
...
- Occurrence intercepts reflect realistic prevalence (e.g., 0–9)
- Detection estimates are species-specific but interpretable

Still, community-Level covariates have 95% credible intervals that include zero. This doesn’t mean the covariates are unimportant, but it does mean that there are not strong community-wide trends. Instead, focus on species-specific effects 
...
Species-level effects revealed more pronounced patterns. ASV_7 (roughtail stingray) showed significant positive detection effects for both filtration volume (mean = 1.39, 95% CI: 0.05 to 3.03, Rhat = 1.0024, ESS = 5,004) and sampling day (mean = 7.80, 95% CI: 3.63 to 14.67, Rhat = 1.0054, ESS = 2,472), indicating increased detectability later in the sampling period and with greater filtered volume.

Spatial covariance parameters phi-1 and phi-2 were estimated at 402.7 (95% CI: 19.8 to 780.5, Rhat = 1.0002, ESS = 14,297) and 400.1 (95% CI: 20.5 to 781.4, Rhat = 1.0001, ESS = 14,121), respectively, indicating spatial autocorrelation with a range of approximately 400 meters.

Model diagnostics supported overall fit and convergence. Most parameters had Rhat values close to 1.00 and high ESS values, indicating good MCMC performance. Posterior predictive checks using Freeman-Tukey statistics showed Bayesian p-values of 0.32 (site level) and 0.30 (replicate level) at the community level, suggesting no major evidence of model misfit. The WAIC was 81.92, with an effective number of parameters (pD) of 16.12, indicating modest model complexity appropriate for a small set of species.



*Discussion* 
The lack of statistically significant community-level effects suggests that occupancy patterns for elasmobranchs were not strongly driven by any single environmental or habitat covariate. However, the species-specific detection responses of ASV_7 (roughtail stingray) indicate that this taxon may be more responsive to sampling effort (filtration volume) and temporal variation (day of sampling). These detection trends could reflect movement behavior, diel activity, or localized aggregations increasing eDNA signal strength.

Of particular interest is the spatial covariance estimate of approximately 400 meters, indicating the range over which spatial autocorrelation in occupancy patterns persists. This means that sites located more than 400 meters apart tend to exhibit distinct occupancy patterns, reflecting ecological or environmental heterogeneity that is not simply due to spatial proximity. Notably, this pattern emerges even for elasmobranchs, highly mobile species relative to many of the other taxa detected in our study. The fact that we still observe spatially distinct patterns in occupancy suggests that their distribution, or the persistence and transport of their eDNA signal, is shaped by localized environmental conditions or behaviors such as site fidelity or habitat preference. These findings validate the spatial resolution of our sampling and reinforce the importance of broad spatial coverage when monitoring even mobile or wide-ranging species with eDNA within a restricted waterbody such as a shallow water lagoon.

Model diagnostics reinforce confidence in these findings. Convergence was strong, posterior predictive checks revealed no major misfit, and WAIC supported the model's ability to balance fit and complexity. Compared to the non-spatial model, the spatial framework provides a better lens to detect subtle ecological signals and interpret species–environment relationships in estuarine systems.

Collectively, these results highlight the value of incorporating spatial structure, even for wide-ranging species, and validate the spatial resolution of your sampling approach. For eDNA-based monitoring in shallow lagoons, accounting for spatial autocorrelation is crucial to avoid overlooking localized patterns that may be critical for species management and conservation.
...
*Spatial Covariance*
The spatial latent factors showed strong spatial structuring, with estimated spatial decay parameters (ϕ) of ~400 meters (phi-1: mean = 399.7, 95% CI: 17.9 to 782.1, Rhat = 1.0001; phi-2: mean = 400.1, 95% CI: 19.1 to 783.1, Rhat = 1.0000). This suggests that species occupancy patterns were autocorrelated at distances up to ~400 meters.
          Mean       SD    2.5%      50%    97.5%   Rhat   ESS
...
*Results text* incorporating community level , species level, spatial variance from mifish--

At the community level, temperature was the only covariate with a statistically supported effect on occupancy (mean = 1.06, 95% CI: 0.22 to 2.14, R̂ = 1.0046, ESS = 501), indicating higher occupancy in warmer waters. Other covariates, such as the habitat categories, eel grass (mean = 0.96), and oyster reef (mean = –1.90), showed suggestive but non-significant trends.

Detection probability was positively associated with later sampling times (mean = 0.34, 95% CI: 0.05 to 0.64, R̂ = 1.0012, ESS = 5534), suggesting that detection improved over the course of the day. Filtration volume had a modest but non-significant positive effect (mean = 0.17, CI: –0.12 to 0.48).

Model diagnostics indicate good overall fit. Posterior predictive checks using the Freeman-Tukey statistic yielded Bayesian p-values of 0.38 and 0.43 (group=replicates), suggesting no major discrepancies between observed and predicted data. The WAIC was 1523.8 with effective number of parameters (pD) = 173.7, supporting model adequacy given the model complexity and number of taxa.

Spatial covariance analysis revealed strong spatial autocorrelation in species occupancy, with decay distances estimated around 400 meters (ϕ₁ = 399.7 m, CI: 17.9 to 782.1; ϕ₂ = 400.1 m, CI: 19.1 to 783.1; both R̂ < 1.0002, ESS > 12,000). This indicates that species occupancy patterns were structured over relatively short distances, with spatial correlation diminishing beyond ~400 meters.

At the species level, occupancy varied by habitat and environmental conditions. Habitat associations revealed clear trends: all ASVs had positive mean coefficients for eel grass, with many showing high effect sizes (e.g., ASV_20 = 1.50, ASV_17 = 1.41), suggesting eelgrass promotes occupancy, even though credible intervals included zero. In contrast, oyster reef habitats were uniformly associated with lower occupancy, with all ASVs showing large negative coefficients. The clam sanctuary showed mostly negative trends, though a few species such as ASV_12 (Spot) had positive but highly uncertain effects.

Temperature effects were consistently positive across ASVs. Species such as ASV_14 (Northern sennet) and ASV_58 (Shads) had strong positive means (>1.5), suggesting temperature sensitivity. These species may be near their lower thermal tolerance limits in this region (REFs), making it sensible that they are sensitive to temperature gradients towards the end of summer.

Depth was generally negatively associated with occupancy. ASV_5 (Atlantic silverside) showed the most negative depth relationship (mean = –1.79, CI: –4.46 to 0.20), suggesting preference for shallower habitats.

Turbidity and chlorophyll both showed broad negative trends. ASVs 4 (Seaboard goby), 6 (Anchovies), and 12 (Spot) showed particularly negative turbidity responses. The chlorophyll signal was more diffuse but consistently negative across all ASVs, potentially reflecting avoidance of eutrophic or bloom-prone areas.

Salinity effects varied, as expected in an estuarine system. Some species (e.g., ASV_7 – Black sea bass) showed strong positive responses to higher salinity (mean = 2.38, CI: 0.28 to 5.40).

Night vs. day sampling effects were mixed. Some species like ASV_2 (Scombriformes) and ASV_58 (Shads) had positive associations with nighttime sampling, while ASV_7 (Black sea bass) and ASV_17 (Northern pipefish) were more common during the day. No consistent community-wide pattern emerged.

Detection probability was also influenced by start time, with ASV_4 (Seaboard goby) and ASV_5 (Atlantic silverside) showing improved detectability later in the day. Filtration volume generally increased detection, with ASV_9 (Tautog) showing a significant positive effect (mean = 0.81, CI: 0.00 to 1.93).




*Paired discussion* This spatial occupancy analysis revealed clear environmental gradients and habitat preferences shaping the teleost community in Shinnecock Bay. The strong positive effect of temperature on species occupancy suggests that thermal optima can structure community composition and species distributions. The reduced occupancy in the western bay likely reflects anthropogenic pressures and lower habitat quality relative to the eastern bay, as established in previous studies (REF). Despite wide credible intervals in some cases, the consistency of directional trends across taxa, particularly for temperature, eel grass habitat, turbidity, and chlorophyll, suggests that ecological filtering is occurring across these gradients.

The strong spatial structuring (ϕ ~400 m) is a key result, indicating that eDNA-based occupancy patterns change meaningfully across short distances, even for mobile taxa. Rather than simply reflecting detection noise, these spatial signals likely capture real ecological variation associated with habitat patchiness, microenvironmental gradients, or behavioral site fidelity. Importantly, the ~400-meter range corresponds to a spatial scale that is ecologically tractable and relevant for management planning in coastal restoration and estuarine monitoring.

The posterior predictive checks and WAIC statistics indicate that the spatial model is both well-fitted and informative. Bayesian p-values clustered near 0.4 to 0.5 suggest no major lack of fit, and the WAIC reflects a reasonable trade off between fit and model complexity for a community data set spanning 52 taxa.

Habitat type played a defining role in species distribution patterns, especially in structured habitats like eel grass beds. While few species-level effects reached statistical significance, the consistent and often large positive posterior means across all ASVs at eel grass sites suggested a community-wide increase in occupancy. Species such as anchovies, northern pipefish, scup, and striped bass were among the strongest positive responders, underscoring the ecological value of submerged aquatic vegetation for supporting biodiversity. Interestingly, spot showed the strongest positive association with the clam sanctuary habitat. As a known predator of mollusks (REF), its presence may reflect prey availability rather than structural habitat quality. Most other taxa were less likely to occur in clam sanctuary sites, which lack physical structure and may therefore support fewer niches for pelagic species.

In contrast, oyster reef habitats showed surprisingly consistent negative occupancy effects across nearly all taxa, including Atlantic silverside, anchovies, and winter flounder. While these effects were not statistically strong for individual species, the directional consistency is notable. One possibility is that biogeochemical or structural filtering at these sites, such as low oxygen microzones or compact reef structure, may reduce detectability or exclude certain species. However, these results should not be interpreted as evidence against oyster restoration. Oyster reefs provide substantial ecosystem services, including shoreline stabilization, water filtration, and long-term habitat enhancement. The reduced occupancy observed here may reflect early successional reef stages, hyper-localized conditions, or methodological artifacts related to eDNA persistence or hydrodynamics. Continued monitoring as reefs mature is essential for capturing longer-term trends in habitat value.

One novel dimension of this study was the inclusion of nighttime sampling, which, though producing weak species-level shifts overall, revealed slightly higher occupancy probabilities for a few taxa. These included Atlantic menhaden and shads, species known or suspected to exhibit diel behavioral patterns such as vertical migration or nocturnal foraging (REF). While not statistically conclusive, these trends point to the potential value of expanding temporal coverage in eDNA surveys, especially for taxa with crepuscular or nocturnal activity patterns that are poorly captured during daytime-only sampling protocols.

Anthropogenic impacts were also detectable in the species-level results, most notably through the strong and consistent negative associations with turbidity and chlorophyll-a. Taxa such as seaboard goby, anchovies, and spot showed steep declines in occupancy with increasing turbidity, suggesting reduced habitat quality, direct physiological stress, or decreased eDNA detectability in more eutrophic conditions. In the case of spot, this may also be driven by preference for clam sanctuaries, which would be lower turbidity due to filtration. These findings align with community-level results and highlight the ecological footprint of nutrient enrichment and sediment disturbance in coastal systems. Additionally, the presence of offshore pelagic species such as Atlantic bluefin tuna and Bigeye tuna at inner-bay sites raises the possibility of anthropogenic eDNA sources—such as runoff or discharge from restaurants or marinas that handle fish from distant waters. MENTION SPECIFIC LOCATIONS While speculative, this hypothesis underscores the importance of source validation in interpreting eDNA results in urbanized estuaries.

Other environmental covariates produced more modest but ecologically meaningful trends. Salinity gradients, while narrow across the system (27–31 psu), appeared to structure species occurrence, with taxa like Black sea bass showing clear preferences for more saline sites. This finding reinforces the importance of estuarine gradients as drivers of faunal composition, even at relatively fine scales. Geographic differences between the eastern and western bays were also evident, with several species (e.g., summer flounder, tautog, darter goby) more likely to occupy eastern sites. These trends likely reflect cumulative differences in habitat quality, development intensity, and proximity to the inlet, and they mirror patterns reported in prior field surveys of Shinnecock Bay (REF).

Overall, this analysis illustrates how integrating environmental covariates, habitat features, and spatial structure into eDNA models can uncover nuanced ecological patterns and provide decision-relevant insights for habitat restoration and biodiversity monitoring.
...
- These Bayesian p-values are near the ideal midpoint (~0.5), suggesting a good model fit at the community level. Values between about 0.2–0.8 generally indicate a healthy balance between model fit and complexity. Thus, your community-level checks are quite good.
- Species-level Bayesian p-values:
  - Almost all species have Bayesian p-values comfortably in the acceptable range (mostly between 0.1–0.8). A few species have values closer to extremes (e.g., ASV_89 = 0.086), but these are relatively rare. This slight deviation is expected with a large number of species.
...
- R̂ (Rhat): mostly close to 1.0 (≤1.05) across community-level parameters.
...
- The spatial covariance (ϕ) parameter ESS > 10,000 is reassuring, indicating stable estimation of spatial structure.
...
In the spatial multi-species occupancy analysis of the CO1 eDNA dataset, several environmental covariates exhibited statistically supported effects on occupancy. Temperature showed a strong positive community-level effect (mean = 0.46, 95% CI: 0.33–0.59, R̂ = 1.046, ESS = 1041), suggesting higher species occupancy in warmer waters. Salinity also positively influenced occupancy (mean = 0.73, 95% CI: 0.57–0.89, R̂ = 1.004, ESS = 822), reflecting the marine affinities of many taxa in Shinnecock Bay. Conversely, nighttime sampling (mean = –1.53, 95% CI: –1.74 to –1.32, R̂ = 1.009, ESS = 1010) and turbidity (mean = –0.23, 95% CI: –0.34 to –0.13, R̂ = 1.002, ESS = 1311) had negative effects on occupancy.

Habitat associations were mixed. Eelgrass beds were associated with increased occupancy (mean = 0.90, 95% CI: 0.47–1.33), while clam sanctuaries (mean = –0.59, CI: –1.04 to –0.14) and oyster reefs (mean = –0.51, CI: –1.04 to 0.04) showed negative community-level occupancy These trends indicate some functional differentiation between habitats but with considerable species-specific variability.

Detection probability was positively associated with both filtration volume (mean = 0.20, 95% CI: 0.15–0.24) and later sampling times during the day (mean = 0.16, 95% CI: 0.11–0.21). No effect was detected for sampling day.

Spatial covariance analysis indicated a decay distance of approximately 446 meters (ϕ = 446.2, 95% CI: 118.7–784.7, R̂ = 1.0004, ESS = 14,406), indicating that species occupancy was spatially structured at sub-kilometer scales.

Posterior predictive checks using the Freeman–Tukey statistic produced Bayesian p-values of 0.38 (group = 1) and 0.43 (group = 2), indicating adequate model fit. Model diagnostics indicated robust convergence for most community-level parameters, with R̂ values mostly near 1.00 and ESS >500.

*Species-Level Trends*

Species-specific analyses revealed fine-scale ecological patterns. Temperature and salinity were positively associated with occupancy across many ASVs. For instance, ASV_35_c (Mnemiopsis leidyi) and ASV_1823_a (Gracilaria tikvahiae) had strong positive temperature effects, reflecting their known thermal tolerance and likely tropical or warm-temperate affinity.

Nighttime sampling was negatively associated with occupancy for 292 ASVs, nearly all phytoplankton, likely reflecting light-dependent detection dynamics. No CO1 taxa had significantly higher occupancy at night. 

A subset of 8 ASVs showed negative associations with day-of-sampling whereas 10 ASVs showed positive associations with sampling day. Interestingly, all ASVs with a negative associated were phytoplankton while the 10 ASVs that had a positive association were mostly animals, (e.g., Nanomia bijuga, ASV_41_b), suggesting time-sensitive detection, possibly related to tide or oceanic influx. Volume of water filtered significantly increased detection probability for 26 ASVs, underscoring the importance of sufficient sampling effort for eDNA recovery.

At the habitat level, three ASVs had strong positive associations with eelgrass: ASV_694_a (Cheilostomatida), ASV_4483_a (Anaulales), and ASV_3893_a (Entomophthoromycetes). Western bay effects were especially strong: 86 ASVs had significantly lower occupancy in the western bay, including Mercenaria mercenaria, Mytilus trossulus, and multiple potential HAB-forming taxa (e.g., Aureococcus anophagefferens, Pseudochattonella farcimen). Only four ASVs showed preference for the western bay, notably Gemma gemma and Grateloupia spp.

*Discussion*
This analysis confirms that spatially structured, environmentally mediated patterns in biodiversity are detectable using eDNA in a shallow, dynamic estuary like Shinnecock Bay. Despite skepticism that such systems are too well-mixed to retain ecological signal, the estimated spatial autocorrelation range of ~446 meters reveals that occupancy varies meaningfully at sub-kilometer scales. This provides compelling evidence against the assumption that small estuaries are too homogenized for spatial eDNA inference.

Temperature and salinity were consistently strong predictors of occupancy, consistent with known physiological preferences across marine phyla. The presence of warm-water-affiliated species, including tropical macroalgae (Gracilaria, Hypnea), calcifying dinoflagellates (Thoracosphaera), and bloom-forming taxa (Takayama, Skeletonema), supports recent evidence of poleward species shifts and seasonal stratification in temperate estuaries.

The species-level trends further show that potential behavioral and environmental variability, including diel shifts, bloom dynamics, and water mass changes, can be resolved through well-replicated eDNA sampling. The prevalence of strong daytime associations for phytoplankton and diatoms, coupled with weak to absent nighttime occupancy, likely reflects diel growth patterns of phytoplankton as well as degradation kinetics tied to light and temperature. The day-of-sampling effects for some taxa (e.g., siphonophores, cryptophytes) may reflect increased oceanic flushing during the later sampling period. Notably, a shift to cooler temperatures in the latter portion of the sampling period was also observed using the continuous CTD measurements.

Habitat trends add a new dimension to these results. The positive eelgrass signal, though driven by only three CO1 ASVs, included taxa like bryozoans and diatoms known to associate closely with submerged vegetation. Meanwhile, the negative oyster reef effects across many ASVs echo trends observed in the MiFish dataset and warrant further exploration. These patterns may reflect early reef successional stages or localized eDNA retention limitations, not necessarily true absence. T

Most strikingly, the western bay signal, characterized by lower occuapncy of many taxonomic groups relative to the eastern bay, aligns with ecological degradation patterns observed in previous field studies. The eastern bay, which benefits from stronger tidal flushing through the inlet, has shown more rapid recovery in key ecological indicators (REF). This study supports that trend, revealing consistently higher biodiversity in the eastern bay, including a greater diversity of phytoplankton. Notably, this includes several taxa associated with harmful algal blooms (HABs). Rather than indicating ecological stress, their presence may signal a resilient and dynamic ecosystem capable of supporting a broad range of functional groups without tipping into bloom conditions. The absence of recent bloom events despite the presence of these taxa (REF) suggests that the eastern bay is in a sufficiently healthy state to accommodate potentially opportunistic species while maintaining overall ecological balance.

Taken together, these results show that spatially explicit eDNA modeling, when coupled with environmental covariates, reveals nuanced ecological signals in small, dynamic coastal lagoons. The strong spatial decay and structured environmental associations underscore the method’s ecological fidelity and its utility for fine-scale biodiversity monitoring, habitat assessment, and restoration planning.
...
exp_mifish_glommed_clr <- data.frame(compositions::clr(otu_df_exp_mifish))
exp_elas_glommed_clr <- data.frame(compositions::clr(otu_df_exp_elas))
exp_co1_glommed_clr <- data.frame(compositions::clr(otu_df_exp_co1))
...
```
First two axes are moderate for Elas02 and CO1 but very weak for Mifish.
...
```
The CO1 PCA plots are generating lines, which indicates that there are strong linear relationships in the data. [This could happen if there is the same value across many samples for one or more variables.](http://rstudio-pubs-static.s3.amazonaws.com/476142_414f269078a44955ad85365a10b8b2ed.html). Elas02 distribution is also looking ~linear. I suspect this is due many zeroes in the data. Be cautious and look for better method to handle zeroes
...
Latitude, Bayside (note these are related!)
...
Latitude, Longitude, Salinity, Bayside



#### CO1
the `sample_data(ps2024_exp_co1)` does not have the calculated avg temp, etc. Grab it from the `metadata_mifish_exp` bc it is exactly the same metadata for each sample
```{r}
# Make into dfs and clean up so names are in same format
metadata_co1_exp <- data.frame(sample_data(ps2024_exp_co1))%>% data.frame() %>% 
  rownames_to_column(var = "SampleID") 

metadata_temp <- data.frame(sample_data(ps2024_exp_mifish))%>% data.frame() %>% 
  rownames_to_column(var = "SampleID") 

# Match up by name, which is in different format for each...
# Step 1: Clean the variables
metadata_co1_exp_clean <- metadata_co1_exp %>%
  mutate(Name_Deploy_Cartr_clean = gsub("\\.", "_", gsub("_$", "", as.character(Name_Deploy_Cartr))))

metadata_temp_clean <- metadata_temp %>%
  rename(Name_Deploy_Cartr_clean = Name.Deploy_Cartr)

# Step 2: Join by the cleaned key
joined_df <- left_join(metadata_co1_exp_clean, metadata_temp_clean, by = "Name_Deploy_Cartr_clean")



# sort metadata in same order as the pca matrix
#metadata_co1_exp <- sample_data(ps2024_exp_co1) %>% data.frame() %>% 
  rownames_to_column(var = "SampleID") %>% arrange(factor(SampleID, levels = rownames(lograt_pca_co1_exp$x)))
...
# Remove some columns of metadate- only retain the env data that I am testing. Leave out the repetitive ID columns, etc.
metadata_co1_exp <- select(metadata_co1_exp, -"Name_Deploy_Cartr", -Deployment, -Cartridge_ID, -sites, -replicates, -site_altname, -controls, -Date, -Year, -Month, -Day, -Start_Time__ET_, -End_Time__ET_)
...
## NMDS
Compositional approach, an NMDS on CLR-tranformed data using the Euclidean distances (aka Aitchison distance)

# PAUSED HERE 5/27
spent a long time figuring out why clr transformation using 2 different packages was giving different distance matrices. Finall figured it out using practice dataset, dietswap, below. Next apply to real data:
```{r}
###
