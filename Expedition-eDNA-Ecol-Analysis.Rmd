...
## PAUSED HERE 7/7- continue going through summary file. load file below to catch up
re-run CO1 model with one factor to improve convergence for phi parameters

#### Save
```{r}
# the data lists and model outputs are huge and slow up saving/loading. remove before running- will just have to rerun making the lists every time. models saved above as their own separate files
...
## PAUSED HERE 6/22- continue going through summary file. load file below to catch up
re-run model to improve convergence for phi parameters

#### Save
```{r}
# the listed objects are huge and slow up saving/loading. remove before running- will just have to rerun making the lists everytime
...
##### Interesting results 
*Community Level*
from summary (focus on parameters with ~ high posterior means [+ or -], 95% credible intervals that do not overlap zero (or barely do), and acceptable convergence diagnostics (Rhat < ~1.1 and ESS > ~200))
