...
*Text Summary*
The non-spatial multi-species occupancy model for the elasmobranch primer set did not identify any statistically significant covariate effects at the community level for occupancy or detection. All 95% credible intervals for predictors such as habitat type, temperature, salinity, and sampling time overlapped zero. However, at the species level, ASV_7 (roughtail stingray) exhibited strong positive detection effects. Filtration volume significantly improved detection probability (mean = 1.83, 95% CI: 0.31–4.54), and day of sampling had an even more pronounced effect (mean = 13.32, 95% CI: 5.66–28.58). Other ASVs showed no statistically significant species-level occurrence or detection responses. While Rhat values and ESS were generally within acceptable ranges, many effects were associated with wide credible intervals, reflecting underlying uncertainty in parameter estimates.

*Discussion*
These results from the non-spatial model indicate that most environmental covariates do not exert a consistent influence on elasmobranch occupancy or detection when spatial structure is ignored. The significant positive effect of filtration volume and sampling day on ASV_7 detection suggests that this species is particularly responsive to sampling effort and timing—factors that may reflect short-term changes in eDNA concentration. However, the absence of other significant patterns and the wide credible intervals point to potential model limitations in resolving ecological signals from noise without accounting for spatial autocorrelation. 
These limitations underscore the importance of incorporating spatial structure into occupancy models, especially in ecosystems with fine-scale habitat gradients or species exhibiting localized distributions. Transitioning to a spatial model is likely to yield more robust insights into the drivers of elasmobranch presence and detection dynamics...
...
re-run all and save model output as their own objects bc slowing down save/load
go back and do posterior checks and report WAIC, etc


#### Save
```{r}
# the data lists are huge and slow up saving/loading. remove before running- will just have to rerun making the lists every time
rm(elas.exp.list,mifish.exp.list,co1.exp.list,sf.elas.out,sf.mifish.out,sf.co1.out)
