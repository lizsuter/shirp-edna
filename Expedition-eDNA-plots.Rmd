...
M. henlei is likely M. canis. It is very low abundance but also likely wrong tax assignment because M. henlei is from the Pacific. Remove (it is a small proportion of total reads- won't change much)


```{r}
# ps2024_exp_mifish <-   subset_taxa(ps2024_exp_mifish, !Species == 'Mustelus henlei')
nrow(tax_table(ps2024_exp_mifish)) # number of taxa left
as.data.frame(unique(tax_table(ps2024_exp_mifish)[, ranks[4]]))
as.data.frame(unique(tax_table(ps2024_exp_mifish)[, ranks[7]]))

ps2024_exp_elas <-   subset_taxa(ps2024_exp_elas, !Species == 'Mustelus henlei')
nrow(tax_table(ps2024_exp_elas)) # number of taxa left
as.data.frame(unique(tax_table(ps2024_exp_elas)[, ranks[4]]))
as.data.frame(unique(tax_table(ps2024_exp_elas)[, ranks[7]]))

# ps2024_trawl_mifish <-   subset_taxa(ps2024_trawl_mifish, !Species == 'Mustelus henlei')
nrow(tax_table(ps2024_trawl_mifish)) # number of taxa left
as.data.frame(unique(tax_table(ps2024_trawl_mifish)[, ranks[4]]))
as.data.frame(unique(tax_table(ps2024_trawl_mifish)[, ranks[7]]))

ps2024_trawl_elas <-   subset_taxa(ps2024_trawl_elas, !Species == 'Mustelus henlei')
nrow(tax_table(ps2024_trawl_elas)) # number of taxa left
as.data.frame(unique(tax_table(ps2024_trawl_elas)[, ranks[4]]))
as.data.frame(unique(tax_table(ps2024_trawl_elas)[, ranks[7]]))
```
...
Expedition/Elas02: 30
SeptemberTrawl/MiFish: 80
SeptemberTrawl/Elas02: 30



##### Make bar plots of top species detected for entire libraries
Expedition/ Mifish
```{r}
# Aggregate at the "Species" level
ps2024_exp_mifish_species <- tax_glom(ps2024_exp_mifish, taxrank = "Species") 
...
exp_mifish_species_abundance <- exp_mifish_species_abundance %>%
  ...
  slice_head(n = 10) # Show top 10 species
...
```{r}
# Aggregate at the "Species" level
ps2024_exp_elas_species <- tax_glom(ps2024_exp_elas, taxrank = "Species") 
...
```{r}
# Aggregate at the "Species" level
ps2024_trawl_mifish_species <- tax_glom(ps2024_trawl_mifish, taxrank = "Species") 
...
```{r}
# Aggregate at the "Species" level
ps2024_trawl_elas_species <- tax_glom(ps2024_trawl_elas, taxrank = "Species") 
...
Join the four datasets and add in common names
```{r}
...
most_abund_all4_datasets <- left_join(most_abund_all4_datasets, commonnames, by = "Species")
...
```{r}
# Fix colors so they remain constant
myColors_sciname <- setNames(c("steelblue4","#DF536B","#61D04F","#2297E6","#28E2E5","#CD0BBC", "#F5C710","brown","#6A3D9A","#FF7F00","gray","orchid1","#FDBF6F","gold1"), commonnames$Species[which(commonnames$Species %in% unique(c(most_abund_all4_datasets$Species, most_abund_all4_datasets$Species)))])

myColors_comname <- setNames(c("steelblue4","#DF536B","#61D04F","#2297E6","#28E2E5","#CD0BBC", "#F5C710","brown","#6A3D9A","#FF7F00","gray","orchid1","#FDBF6F","gold1"),commonnames$CommonName[which(commonnames$Species %in% unique(c(most_abund_all4_datasets$Species, most_abund_all4_datasets$Species)))])
...
top_species_abundance_plot <- ggplot(most_abund_all4_datasets, aes(x = sample_type, y = RelativeAbundance, fill = CommonName)) +
  ...
  theme_minimal() +
  guides(fill = guide_legend(ncol = 2)) # Adjust legend columns if needed
