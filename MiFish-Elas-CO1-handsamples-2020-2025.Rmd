...
Some sample_data files have incorrect entries for lat long, east west, etc. Grab this from the stations metadata db which is manually and frequently updated
```{r}
stations_master <- readr::read_csv("../eDNA-databases/stationsmetadata.csv",
                                  show_col_types = FALSE)

fix_sample_metadata <- function(sample_md, stations_md = stations_master) {

  stations_lu <- bind_rows(
    stations_md %>%
      transmute(join_key = as.character(site_altname),
                sites = as.character(sites),
                site_altname, lat, long, Station = name, Bayside, Habitat, `Site-type`),
    stations_md %>%
      transmute(join_key = as.character(sites),
                sites = as.character(sites),
                site_altname, lat, long, Station = name, Bayside, Habitat, `Site-type`)
  ) %>%
  ...
  sample_md %>%
    mutate(join_key = as.character(sites)) %>%
    select(-any_of(c("sites","site_altname","Station","lat","long","Bayside","Habitat","Site-type","Site.type"))) %>%
    left_join(stations_lu, by = "join_key") %>%
    select(-join_key) %>%
    rename(`Site.type` = `Site-type`)
}
...
sample_metadata <- fix_sample_metadata(sample_metadata, stations_master)
...
sample_metadataa <- fix_sample_metadata(sample_metadataa, stations_master)
sample_metadatab <- fix_sample_metadata(sample_metadatab, stations_master)
...
sample_metadataa <- fix_sample_metadata(sample_metadataa, stations_master)
sample_metadatab <- fix_sample_metadata(sample_metadatab, stations_master)
sample_metadatac <- fix_sample_metadata(sample_metadatac, stations_master)
...
## Retain a total CO1 dataset
Below I am cleaning up CO1 to match it directly to trawl dataset (and naming it `_fortrawlcomparisononly`) but first retain. a full CO1 dataset with phytoplankton, all arthropods, echinoderms, etc for other analyses (invasive species, HABs, for example)...

```{r}
ps_co1_2024_total <- ps_co1_2024
ps_co1_2025_total <- ps_co1_2025
...
Remove teleost fish from elas02
...
Start working with both CO1 datasets again (trawl only and total)
...
taxonomy_co1_2024_total <- as.data.frame(tax_table(ps_co1_2024_total)) %>%
  rownames_to_column("ASV") %>%
  left_join(commonnames, by = c("Species" = "Taxon"))
row.names(taxonomy_co1_2024_total) <- taxonomy_co1_2024_total$ASV
taxonomy_co1_2024_total <- taxonomy_co1_2024_total %>% select(-ASV)
tax_table(ps_co1_2024_total) <- as.matrix(taxonomy_co1_2024_total)
...
taxonomy_co1_2025_total <- as.data.frame(tax_table(ps_co1_2025_total)) %>%
  rownames_to_column("ASV") %>%
  left_join(commonnames, by = c("Species" = "Taxon"))
row.names(taxonomy_co1_2025_total) <- taxonomy_co1_2025_total$ASV
taxonomy_co1_2025_total <- taxonomy_co1_2025_total %>% select(-ASV)
tax_table(ps_co1_2025_total) <- as.matrix(taxonomy_co1_2025_total)
...
as.data.frame(tax_table(ps_co1_2024_total)) %>% rownames_to_column("ASV") %>% filter(is.na(CommonName))
as.data.frame(tax_table(ps_co1_2025_total)) %>% rownames_to_column("ASV") %>% filter(is.na(CommonName))

```



a list of species that are missing from the common names database- update db as needed
```{r}
as.data.frame(tax_table(ps_co1_2024_total)) %>% rownames_to_column("ASV") %>% filter(is.na(CommonName)) %>% select(Species) %>% unique()
as.data.frame(tax_table(ps_co1_2025_total)) %>% rownames_to_column("ASV") %>% filter(is.na(CommonName)) %>% select(Species) %>% unique()
```

I will never be able to do all CO1 species but added in common names for some important ones, like Aureococcus anophagefferens and other HABs and invasives. Continue to supplement each year.
...
For CO1, use 0 for now. There is some guinea pig bleedover in 2025 samples (max 132 reads) but real/ rare species are very low abundance, like Blue crab, Crangon species, others...I want to see these detections for now.
...
plot_bar(ps_co1_2024_total)
plot_bar(ps_co1_2025_total)

```

Cut out any sample with <10,000 reads from all libraries
...
ps_co1_2024_total <- prune_samples(sample_sums(ps_co1_2024_total)>=100,ps_co1_2024_total)
ps_co1_2025_total <- prune_samples(sample_sums(ps_co1_2025_total)>=100,ps_co1_2025_total)
```
...
ps_co1_2024_total_glommed <- ps_co1_2024_total
ps_co1_2025_total_glommed <- ps_co1_2025_total
...
tax_table(ps_co1_2024_total_glommed) <- tax_table(ps_co1_2024_total_glommed)[,c(1, 7)]
tax_table(ps_co1_2025_total_glommed) <- tax_table(ps_co1_2025_total_glommed)[,c(1, 7)]
...
ps_co1_2024_total_glommed <- tax_glom(ps_co1_2024_total_glommed, "Species")
ps_co1_2025_total_glommed <- tax_glom(ps_co1_2025_total_glommed, "Species")
...
data.frame(tax_table(ps_co1_2024_total)) # to compare
data.frame(tax_table(ps_co1_2024_total_glommed))
data.frame(otu_table(ps_co1_2024_total_glommed))

data.frame(tax_table(ps_co1_2025_total)) # to compare
data.frame(tax_table(ps_co1_2025_total_glommed))
data.frame(otu_table(ps_co1_2025_total_glommed))
...
#2024
co1_2024_asv_table <- data.frame(otu_table(ps_co1_2024_total))
write.csv(co1_2024_asv_table,"filtered_data/co1_2024_asv_table_total.csv", row.names = TRUE)

co1_2024_tax_table <- rownames_to_column(data.frame(tax_table(ps_co1_2024_total)), var = "ASV")
write.csv(co1_2024_tax_table,"filtered_data/co1_2024_tax_table_total.csv", row.names = FALSE)

co1_2024_sample_data <- data.frame(sample_data(ps_co1_2024_total))
write.csv(co1_2024_sample_data,"filtered_data/co1_2024_sample_data_total.csv", row.names = TRUE)

co1_2024_glommed_asv_table <- data.frame(otu_table(ps_co1_2024_total_glommed))
write.csv(co1_2024_glommed_asv_table,"filtered_data/co1_2024_glommed_asv_table_total.csv", row.names = TRUE)

co1_2024_glommed_tax_table <- rownames_to_column(data.frame(tax_table(ps_co1_2024_total_glommed)), var = "1stRepresentative_ASV")
write.csv(co1_2024_glommed_tax_table,"filtered_data/co1_2024_glommed_tax_table_total.csv", row.names = FALSE)


#2025
co1_2025_asv_table <- data.frame(otu_table(ps_co1_2025_total))
write.csv(co1_2025_asv_table,"filtered_data/co1_2025_asv_table_total.csv", row.names = TRUE)

co1_2025_tax_table <- rownames_to_column(data.frame(tax_table(ps_co1_2025_total)), var = "ASV")
write.csv(co1_2025_tax_table,"filtered_data/co1_2025_tax_table_total.csv", row.names = FALSE)

co1_2025_sample_data <- data.frame(sample_data(ps_co1_2025_total))
write.csv(co1_2025_sample_data,"filtered_data/co1_2025_sample_data_total.csv", row.names = TRUE)

co1_2025_glommed_asv_table <- data.frame(otu_table(ps_co1_2025_total_glommed))
write.csv(co1_2025_glommed_asv_table,"filtered_data/co1_2025_glommed_asv_table_total.csv", row.names = TRUE)

co1_2025_glommed_tax_table <- rownames_to_column(data.frame(tax_table(ps_co1_2025_total_glommed)), var = "1stRepresentative_ASV")
write.csv(co1_2025_glommed_tax_table,"filtered_data/co1_2025_glommed_tax_table_total.csv", row.names = FALSE)
