...
CO1: Remove all for now. There are mostly contamination (primates, generic mammals, cat, dog/wolf) but there are some interesting taxa here (opposums and bats) but they are not defined to species and not target of study.
...
Ostracoda also marine - seed shrimp - but are too small to be target of the trawl
...
Remove Polyplacophora (chitons), Ascidiacea (sea squirts), Unknown Mollusca (Phylum)- wouldn't be targetted by the trawl
```{r}
...
```
Remove Unknown Malacostraca, Amphipoda, Isopoda, Euphausiacea (krill- zooplankton - too small)

```{r}
ps_co1_2024 <- subset_taxa(ps_co1_2024, !Order == 'Unknown Malacostraca (Class)' & !Order == 'Amphipoda' & !Order == 'Isopoda' & !Order == 'Euphausiacea')

unique(tax_table(ps_co1_2024)[, "Order"])
```

Remove other unknown Molluscs and Arthropods above genus level -
Unknown Veneridae (Family)
Unknown Nuculida (Order)


```{r}
ps_co1_2024 <- subset_taxa(ps_co1_2024, !Species == 'Unknown Veneridae (Family)' & !Species == 'Unknown Nuculida (Order)')
```
...
# CO1 has many more taxa and therefore min abund threshold should be lower. Trim only zeroes (<1) for now
...
ps_co1_2024 <- prune_samples(sample_sums(ps_co1_2024)>=500,ps_co1_2024)
...
write.csv(co1_2024_asv_table,"filtered_data/co1_2024_asv_table.csv", row.names = TRUE)

co1_2024_tax_table <- rownames_to_column(data.frame(tax_table(ps_co1_2024)), var = "ASV")
write.csv(co1_2024_tax_table,"filtered_data/co1_2024_tax_table.csv", row.names = FALSE)

co1_2024_sample_data <- data.frame(sample_data(ps_co1_2024))
write.csv(co1_2024_sample_data,"filtered_data/co1_2024_sample_data.csv", row.names = TRUE)

co1_2024_glommed_asv_table <- data.frame(otu_table(ps_co1_2024_glommed))
write.csv(co1_2024_glommed_asv_table,"filtered_data/co1_2024_glommed_asv_table.csv", row.names = TRUE)

co1_2024_glommed_tax_table <- rownames_to_column(data.frame(tax_table(ps_co1_2024_glommed)), var = "1stRepresentative_ASV")
write.csv(co1_2024_glommed_tax_table,"filtered_data/co1_2024_glommed_tax_table.csv", row.names = FALSE)
